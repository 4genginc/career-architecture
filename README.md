...
This repository defines the **company-level career operating system** used to produce, validate, and maintain resumes (and related career assets) for multiple people.

It is designed to scale across **Rongjun**, **Mingjun**, and (eventually) **Weijie** without mixing personal “source of truth” content.
...
- A **governed documentation system** (CANON + ADRs) explaining how resume work is managed.
- A set of **playbooks, scorecards, templates, and experiments** that make outputs consistent and high quality.
- A place to store **process logs** (weekly reviews, decision logs, outreach logs) that are useful across people.

---

## What this repo is NOT

This repo is **not** the authoritative home for any individual’s resume content.

Do **not** store:

- full personal resumes as the canonical truth
- sensitive personal data (DOB, SSN, addresses, immigration details)
- private employer documentation

Keep person-specific resume truth in **person-level resume repos**.
...
1. **Person-level resume repos (truth)**
   - Example: `resume-rongjun/`, `resume-mingjun/`, `resume-weijie/`
   - Holds `RESUME_MASTER.md` + lane variants + verified facts

2. **Shared workspace repo (assets + exports)**
   - Example: `careers/`
   - Holds shared playbooks plus per-person exported resumes, LinkedIn assets, interview notes
...
- `CANON_ARCH.md`
  - The canonical, defended truth of how career work is organized at the company level.

- `adr/`
  - Architecture Decision Records explaining *why* key decisions were made.

- `governance/`
  - Principles, boundaries, risk register, and changelog for the system.
...
- `logs/`
  - A running history of decisions, weekly reviews, and conversations.
...
### Add a new person (Rongjun / Mingjun / Weijie)
...
## Governance rules (lightweight)

- **CANON first:** if something is “true and defended,” it goes into `CANON_ARCH.md`.
- **Decisions are logged:** if it changes how the system works, record an ADR.
- **Validation required:** lane resumes must be traceable back to a verified master source.
- **Privacy boundary:** personal content stays in person-level repos and/or `careers/people/...`.

---

## How to contribute

- New system decisions → add an ADR + update `adr/ADR-INDEX.md`
- New workflow → add/update a playbook
- New measurement → update `scorecards/METRICS.md`
- New experiment → add `experiments/EXP-###-*.md`
- Weekly cadence → log in `logs/weekly/YYYY-W##.md`
